...
const HomeScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://${ipAddress}:7777/category-service/api/category');
  ...
  const handleCategorySelection = async (categoryId) => {
    ...
    try {
      const response = await axios.get(`http://192.168.113.110:7777/product-service/api/product/searchByIdCategory?idCategory=${categoryId}`);
      setRecipes(response.data);
    } catch (error) {
