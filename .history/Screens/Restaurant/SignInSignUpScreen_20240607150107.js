...
const signUp = async (name, email, password, countryCode, phoneNumber) => {
  try {
    const fullPhoneNumber = `${countryCode}${phoneNumber}`; // Include country code
    const role = 'client'; // Static role
    const response = await axios.post('http://localhost:7777/user-service/api/v1/signup', {
      name,
      email,
      password,
...
const SignUpScreen = ({ setIndex }) => {
  ...
  const handleSignUp = async () => {
    try {
      ...
      showMessage({
        message: 'Success',
        description: 'Sign Up successful',w
        type: 'success',
      });
      setIndex(0);
