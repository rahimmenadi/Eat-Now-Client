    ...
    try {
      ...
      if (!token) {
        Toast.show({
          type: 'error',
          text1: 'Error',
          text2: 'You need to be logged in to add items to the wishlist.',
        });
        return;
      }
      ...
      Toast.show({
        type: 'success',
        text1: 'Success',
        text2: 'Item added to wishlist successfully.',
      });
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to add item to wishlist. Please try again.',
      });
    ...
    try {
      ...
      if (!token) {
        Toast.show({
          type: 'error',
          text1: 'Error',
          text2: 'You need to be logged in to add items to the cart.',
          position: 'bottom',
          visibilityTime: 4000,
          autoHide: true,
          topOffset: 30,
      ...
      const response = await axios.post(
        `http://${ipAddress}:7777/order-service/api/carts/products`,
        {
          productId: recipeId,
          quantity: 1,
        },
