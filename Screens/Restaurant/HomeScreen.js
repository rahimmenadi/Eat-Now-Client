...
const HomeScreen = () => {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      const userWilaya = await AsyncStorage.getItem('userWilaya');
      if (query) {
        const response = await axios.get(`http://${ipAddress}:7777/product-service/api/product/search?name=${query}&wilaya=${userWilaya}`);
        
        setRecipes(response.data);
        
      } else {
        const response = await axios.get(`http://${ipAddress}:7777/product-service/api/product/searchByWilaya?wilaya=${userWilaya}`);
        setRecipes(response.data);
      }
    } catch (error) {
  ...
  const handleCategorySelection = async (categoryId) => {
    ...
    try {
      ...
      if (categoryId === 'all') {
        ...
        const response = await axios.get(`http://${ipAddress}:7777/product-service/api/product/searchByIdCategory?idCategory=${categoryId}&wilaya=${userWilaya}`);
        setRecipes(response.data);
      }
    } catch (error) {
